"""Cosmos 后端 —— A股数据层 HTTP 接口（FastAPI，基于 Vibe-Research MIT 改编）。
...
app = FastAPI(title="Cosmos API", version="0.3.1")
...
@app.get("/api/health")
def health():
    return {"ok": True, "service": "cosmos-api", "version": "0.3.1"}
...
def chat(req: ChatReq):
    ...
    is_cli = cfg["provider"].startswith("cli-")
...
def _check_llm(llm: LLMConfig) -> dict:
    ...
    阶段3（NASDX 融合）：请求体未带完整 API 接入配置时，回退到服务端统一
    配置（llm_cfg：NASDX_* 环境变量优先、VR_* 兜底），使 AI 功能可通过统一
    凭证运行，同时保留前端「接入 AI」页自填能力。CLI 接入（cli-*）不参与
    服务端兜底，维持原行为。
    配置问题走 HTTP 400（前端能弹提示引导去「接入 AI」页），运行时错误留给流内 error 事件。
    """
    cfg = llm.model_dump()

    # CLI 接入：无需 baseURL / apiKey / model，维持原行为
    if cfg.get("provider", "").startswith("cli-"):
        kind = cfg["provider"][4:]
