...
GitHub Actions 的 Windows 桌面检查使用 Node 24 版 `actions/checkout@v5` 与 `actions/setup-python@v6`，并运行 `python -B run_desktop_release_check.py --skip-final-audit --fail-fast`。快速 portable 包默认写到 `dist\NASDX-Desktop-check\`，避免覆盖带 `.venv` 的正式发行包；完整投资数据审计仍以本地 `run_product_readiness.py` 为准。
