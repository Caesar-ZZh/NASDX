...
def _step_command(
    ...
    if step == "refresh":
        return [sys.executable, "-u", str(ROOT / "scripts" / "fetch_stock_data.py")]
    if step == "scan_etf50":
        return [sys.executable, "-u", str(ROOT / "scripts" / "scan_etf50.py")]
    if step == "scan_stocks60":
        return [sys.executable, "-u", str(ROOT / "scripts" / "scan_stocks_full.py")]
    if step == "selector_run":
        return [sys.executable, "-u", str(ROOT / "scripts" / "run_stock_selector.py")]
    if step == "analysis":
        if not stock_code:
