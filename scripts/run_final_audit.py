...
def check_desktop_delivery_assets() -> str:
    ...
    release_text = _read_text(ROOT / "scripts" / "run_desktop_release_check.py")
    completion_text = _read_text(ROOT / "scripts" / "run_desktop_completion_audit.py")
    release_evidence_text = _read_text(ROOT / "scripts" / "run_desktop_release_evidence.py")
    security_text = _read_text(ROOT / "scripts" / "run_security_checks.py")
    ...
    required_source_markers = [
        ...
        (control_text, "scripts/fetch_stock_data.py"),
        ...
        (release_text, "scripts/run_security_checks.py"),
        (release_text, "scripts/run_desktop_doctor.py"),
        (release_text, "scripts/run_desktop_completion_audit.py"),
        (release_text, "scripts/run_desktop_release_evidence.py"),
        ...
        (completion_text, "scripts/run_desktop_release_evidence.py"),
        ...
        (portable_smoke_text, "scripts/run_desktop_doctor.py"),
        (portable_smoke_text, "scripts/run_desktop_completion_audit.py"),
        ...
        (installed_smoke_text, "scripts/run_desktop_doctor.py"),
        (installed_smoke_text, "scripts/run_desktop_completion_audit.py"),
...
def check_portfolio_contract() -> str:
    ...
    for profile in ("conservative", "balanced", "aggressive"):
        ...
        if plan["action_gate"] == "refresh_required":
            ...
            if not any("scripts/run_investment_workflow.py" in item for item in plan["next_actions"]):
                raise AssertionError("数据过期时未给出刷新工作流动作")
    scanner_text = _read_text(ROOT / "scripts" / "scan_stocks_full.py")
...
def check_documentation() -> str:
    ...
    required_readme = [
        ...
        "未来情景推演",
        "scripts/run_investment_workflow.py",
        "scripts/run_portfolio_plan.py",
        "scripts/run_investment_brief.py",
        "scripts/run_position_sizing.py",
        "scripts/run_recommendation_tracker.py",
        "scripts/run_recommendation_review.py",
        "scripts/run_account_review.py",
        "scripts/run_review_snapshot.py",
        "nasdx_history.db",
        "task_id",
        "最终投资简报",
        "资金仓位换算",
